...
while camera.IsGrabbing():
    ...
    if grabResult.GrabSucceeded():
        ...
        x1, y1, width, height = 980, 500, 12, 1  # Monitored area  #560 #565
    ...
    #             # Only print decoded message if it's different from last decoded message
    #             if decoded_message != last_decoded_message:
    #                 print(f"Decoded message: {decoded_message}")
    #                 all_decoded_message.extend(decoded_message)
    #                 last_decoded_message = decoded_message  # Store the last decoded message

    # # Condition to print "all value intensity" when there are 50 data points <= key_start_threshold in row_sums
    # if (
    #     capturing_data
    #     and sum(value <= key_start_threshold[1] for value in row_sums) >= 100
